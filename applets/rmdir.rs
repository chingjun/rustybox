...
pub fn main(args: &[~str]) {
    ...
    for d in matches.free.iter() {
        if recursive {
            ...
            loop {
                match std::io::fs::rmdir(&path) {
                    Err(e) => {
                        common::err_write_line(format!("rmdir: {:s}: {:s}", path.as_str().unwrap(), e.desc));
                        std::os::set_exit_status(1);
                        break;
                    }
                    _ => {}
            ...
            match std::io::fs::rmdir(&Path::new(d.as_slice())) {
                Err(e) => {
                    common::err_write_line(format!("rmdir: {:s}: {:s}", *d, e.desc));
                    std::os::set_exit_status(1);
                }
                _ => {}
            }
